...
get_incremented_semantic_version () {
  ...
  echo "3. semantic version input $current_version"
  if [ -z "$current_version" ]
  then
        current_version="0.0.0"
  fi
  echo "4. checking current version $current_version"
  local semantic_version=$(sm_increment $increment_type $current_version)
...
get_semantic_version_from_git () {
  ...
  echo "5.This is from git tags $git_version"
}
